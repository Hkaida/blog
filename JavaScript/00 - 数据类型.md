...
Symbol 是 ES6 中引入的新基本类型，表示独一无二的值。

Symbol 值通过 Symbol() 函数生成。该函数可以接受一个字符串作为参数，表示对  Symbol 实例的描述，使得在调试代码时容易区分。
...
console.log(typeof s2) // "symbol(foo)"
console.log(typeof s3) // "symbol(bar)"
```
